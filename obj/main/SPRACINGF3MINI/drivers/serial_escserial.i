...
const escSerialConfig_t pgResetTemplate_escSerialConfig __attribute__ ((section(".pg_resetdata"), used, aligned(2))) = { .ioTag = ((ioTag_t)((((1) + 1) << 4) | (4))), }
        ...
        for (volatile uint8_t i = 0; i < 8; i++) {
            if (pwmMotors[i].enabled) {
                if (pwmMotors[i].io != ((IO_t)0)) {
                    for (volatile uint8_t j = 0; j < 12; j++) {
